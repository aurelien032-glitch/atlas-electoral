...
const ordinateur = { largeur: 1460, hauteur: 1080, volet: false }
...
describe('marges du cadrage', () => {
  ...
  it('pose encarts puis légende sur la carte plutôt que de l\'écraser', () => {
    const etroit = { largeur: 700, hauteur: 768, volet: false }
    expect(marges(tout, etroit, 'france')).toMatchObject({ left: 320, right: 72 })
    const tresEtroit = { largeur: 500, hauteur: 390, volet: false }
    expect(marges(tout, tresEtroit, 'france')).toMatchObject({ left: 24, right: 72 })
  })

  it('garde le haut de l\'écran au-dessus du volet, sources comprises', () => {
    const telephone = { largeur: 390, hauteur: 844, volet: true }
    expect(marges(tout, telephone, 'france')).toEqual({ top: 72, bottom: 354 + 36, left: 16, right: 16 })
