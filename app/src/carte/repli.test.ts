...
import { repli, territoiresDesContours } from './repli'

const resultat = { inscrits: 0, votants: 0, blancs: 0, nuls: 0, exprimes: 0, tete: null, egalite: false, avance_x10000: null }
const agregat = (niveau: Agregat['niveau'], code: string): Agregat => ({ ...resultat, niveau, code })
...
const agregats = ['10001', '10387', '33063', '75056'].map((c) => agregat('commune', c))
  .concat(['75101', '75102'].map((c) => agregat('arrondissement', c)))
...
describe('repli à la commune', () => {
  ...
  it("dessine par leur commune les territoires qui n'ont aucun contour, jamais une ville découpée en arrondissements", () => {
    expect([...repli(contours, agregats, null, communeDu).sansDessin].sort()).toEqual(['10387', '75102'])
  })

  it('compte les bureaux sans contour de chaque territoire', () => {
    const r = repli(contours, agregats, [bureau('10001_0001', 100), bureau('10001_0003', 50), bureau('75056_0101', 200)], communeDu)
  ...
  it('montre en entier un territoire dont la plupart des inscrits votent dans un bureau sans contour', () => {
    const r = repli(contours, agregats, [bureau('33063_1001', 400), bureau('33063_1101', 100), bureau('10387_0001', 300)], communeDu)
    expect([...r.aLaCommune].sort()).toEqual(['10387', '33063'])
  })

  it("n'a rien à compter sans les bureaux : la carte s'arrête alors à la commune", () => {
    const r = repli(contours, agregats, null, communeDu)
