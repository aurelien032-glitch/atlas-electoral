...
| `candidats_nuances.csv` | Nuance attribuée aux candidatures sans nuance officielle (présidentielles 2017 et 2022, européennes 2019) : le code de leur parti dans la grille 2026. La colonne `nom` porte le nom du candidat ou, pour une liste, son libellé abrégé. |
| `totaux_officiels.csv` | Totaux nationaux officiels de chaque tour (54 sur 56), pour le test de réconciliation et le rapport qualité : source, lien, périmètre, `fiabilite` (`officielle` : Conseil constitutionnel, ministère, Journal officiel ; `secondaire` : total compilé d'après les fichiers du ministère quand l'État n'en publie pas ; `introuvable`). N'y entrent que des chiffres relus dans leur source. |
| `totaux_officiels_variantes.csv` | Seconde version officielle d'un même total, quand il en existe deux (page des résultats proclamés et fichier du ministère publié en 2013) : nos totaux égalent parfois l'une, pas l'autre. |
