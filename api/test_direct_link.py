...
try:
    ...
    # 2. Check overlap with GIS 'id_bv'
    ov_id_bv = conn.execute(f"""
        SELECT COUNT(DISTINCT r.id_brut_miom)
        FROM '/data/Election/general_results.parquet' r
        JOIN bureaux b ON r.id_brut_miom = b.id_bv
        WHERE r.id_election = '{election_id}'
    """).fetchone()[0]
    print(f"Match results.id_brut_miom == bureaux.id_bv: {ov_id_bv} ({round(ov_id_bv/total_results*100, 2)}%)")
    
    # 3. Check overlap with GIS 'codeBureauVote'
    ...
    # 4. Compare with current REU JOIN
